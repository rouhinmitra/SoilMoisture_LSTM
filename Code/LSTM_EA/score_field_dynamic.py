"""Score the field-dynamic-S2 arms against paper_config_fixed, seed-paired."""
...
def install(mode):
    ...
    s1 = pd.read_csv(f"{FA}/s1_daily_field.csv", parse_dates=["Date"]) if mode=="s2_s1" else None
    def patched(self, fp):
...
rows=[]
for arm,ck,mode in [("baseline",MV/"paper_config_fixed_sensitivity",None),
                    ("fld_s2",MV/"fielddyn_s2","s2"),
                    ("fld_s2_s1",MV/"fielddyn_s2_s1","s2_s1")]:
    install(mode); importlib.reload(A)
    c=cfg(mode=="s2_s1")
...
install(None)
R=pd.DataFrame(rows); R.to_csv(MV/"field_dynamic_scored.csv",index=False)
print(f"\nscored -> {MV/'field_dynamic_scored.csv'} ({len(R)} rows)")
