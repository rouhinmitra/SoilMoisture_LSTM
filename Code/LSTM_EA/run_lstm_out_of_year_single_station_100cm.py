...
at a single station — 100cm root zone soil moisture.

Mirrors `run_lstm_out_of_year_single_station.py` but:
- predicts `RZSM_100_avg` instead of `RZSM_25_avg`
...
def main() -> None:
    ...
    base_config = replace(
        base_config,
        target_col="RZSM_100_avg",
        output_dir="outputs/lstm_out_of_year_single_station_100cm",
    ...
    log.info("=" * 80)
    log.info("LSTM OUT-OF-YEAR (YEAR-WISE HELD-OUT) CV - SINGLE STATION (100cm)")
    log.info("=" * 80)
    ...
    log.info("\n" + "=" * 80)
    log.info("OUT-OF-YEAR PROCESSING (100cm) COMPLETED")
    log.info("=" * 80)
